...
include("connection.php");
    ...
    $date = $_POST['date'];
       ...
       if (!in_array($ext, $allowed)) {
           $msg = "Invalid file type. Only JPG, JPEG and PNG types are accepted.";
           $a_rr=array("status"=>2,"msg"=>$msg);   
           echo json_encode($a_rr);
           return;
       }
...
if($_POST['action']=="INSERT"){
    $sql = "INSERT INTO `latest_news_tbl` ( `gallery`,`title`, `description`, `date`) 
    VALUES ('$GALLERY','$title', '$description', '$date')";
}else
if($_POST['action']=="UPDATE"){
    $sql = "UPDATE `latest_news_tbl`SET 
    `gallery`='$GALLERY',
    `title`='$title', 
